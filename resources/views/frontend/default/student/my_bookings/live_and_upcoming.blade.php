            ...
            <tbody>
                @foreach ($my_bookings as $my_booking)
                    <tr>
                        ...
                        <td>
                            <a href="{{ tution_started($my_booking->id) ? route('tution_class.join', slugify($my_booking->id)) : 'javascript:void(0);' }}"
                                class="d-flex align-items-center justify-content-center btn btn-primary text-18 text-white py-3" data-bs-toggle="tooltip" data-bs-title="{{ get_phrase('Join Now') }}">
...
<div class="entry-pagination">
    <nav aria-label="Page navigation example">
        {{ $my_bookings->appends(['tab' => 'live-upcoming'])->links() }}
    </nav>
</div>
@endif
<!-- Pagination -->